...
static String htmlStreamer() {
    ...
    h += F(
        ...
        "if(nextT<ctx.currentTime+MIN_AHEAD)nextT=ctx.currentTime+MIN_AHEAD;if(nextT>ctx.currentTime+MAX_AHEAD)nextT=ctx.currentTime+TARGET_AHEAD;const src=ctx.createBufferSource();src.buffer=b;src.connect(ctx.destination);src.start(nextT);nextT+=n/rate;setMeter(peak*100);}"
        ...
        "lastSeq=seq||lastSeq;playFrame(rate,pcm);const ahead=Math.max(0,nextT-ctx.currentTime);setStatus('Playing • '+rate+' Hz • buffer '+Math.round(ahead*1000)+' ms • '+frames+' frame'+(frames===1?'':'s')+(dropped?(' • skipped '+dropped):''));"
...
static void httpAudioStatus() {
    ...
    json += "\"filter_detail\":\"Unsigned PCM auto-detection is disabled for this PDM profile; raw samples stay signed before HPF/gain.\",";
...
static void httpSet() {
    ...
    else if (key == "noise_filter") { String v=web.arg("value"); if (v=="on"||v=="off") { noiseFilterEnabled=(v=="on"); saveAudioSettings(); } }
